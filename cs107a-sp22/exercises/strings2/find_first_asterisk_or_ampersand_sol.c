...
char *find_first_asterisk_or_ampersand(char *s) {
  // You _could_ call strchr twice and compare the pointers to figure out which
  // is earlier in the string, but that's less efficient and we technically
  // haven't discussed how to compare pointers like that.
  size_t offset = strcspn(s, "*&");
  // Food for thought: why didn't I use strlen here?
  if (s[offset] == '\0') {
    return NULL;
  }
  return s + offset;
