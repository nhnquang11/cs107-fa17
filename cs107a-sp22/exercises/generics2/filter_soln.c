...
void *filter(void *arr, size_t num_elems, size_t elem_size,
  ...
  for (int i = 0; i < num_elems; i++) {
    void *ith_ptr = (char*)arr + i * elem_size;
    if (should_keep(ith_ptr)) {
      void *next_ptr = (char*)filtered_arr + next_slot * elem_size;
      memcpy(next_ptr, ith_ptr, elem_size);
      next_slot++;
    }
  }
...
void demo_filter_for_strings() {
  ...
  int filtered_size = 0;
  char **filtered_strings = filter(strings, num_strings, sizeof(char*),
      does_not_contain_o, &filtered_size);
