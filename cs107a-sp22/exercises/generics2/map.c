...
void *map(void *arr, size_t num_elems, size_t elem_size, void (*transform)(void *dst, void *src)) {
    void *transformed_arr = malloc(num_elems * elem_size);
    for (int i = 0; i < num_elems; ++i) {
        void *ith_ptr_arr = (char *)arr + i * elem_size;
        void *ith_ptr_trans = (char *)transformed_arr + i * elem_size;
        transform(ith_ptr_trans, ith_ptr_arr);
    }
...
void demo_map_for_ints() {
  ...
  int *mapped_integers = map(integers, num_ints, sizeof(integers[0]), square_transform);
