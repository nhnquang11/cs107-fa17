...
void consolidate_messy_ptr_array(int *arr[], int n) {
  int *consolidated = malloc(n * sizeof(int));
  assert(consolidated != NULL);
  for (int i = 0; i < n; i++) {
    consolidated[i] = *arr[i];
    free(arr[i]);
    arr[i] = consolidated + i;
  }
}
...
void free_messy_ptr_array(int *arr[]) {
  // We're just going to assume n > 0.
  free(arr[0]); // Conveniently points to start of consolidated array.
  free(arr);
}
