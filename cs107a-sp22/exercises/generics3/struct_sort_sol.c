...
int compare_aged_person_by_name_then_age(const void *a, const void *b) {
  struct aged_person *ap1 = *(struct aged_person**)a;
  struct aged_person *ap2 = *(struct aged_person**)b;
  int cmp = strcmp(ap1->first_name, ap2->first_name);
  if (cmp != 0) {
    return cmp;
  }
...
int compare_aged_person_by_age_then_name(const void *a, const void *b) {
  struct aged_person *ap1 = *(struct aged_person**)a;
  struct aged_person *ap2 = *(struct aged_person**)b;
  int cmp = ap1->age - ap2->age;
  if (cmp != 0) {
    return cmp;
  }
  return strcmp(ap1->first_name, ap2->first_name);
}
...
void conditional_sort(struct aged_person **arr, int length, bool by_name_first) {
  // YOU MUST CALL qsort EXACTLY ONCE.
  int (*cmp)(const void*, const void*) = compare_aged_person_by_age_then_name;
  if (by_name_first) {
    cmp = compare_aged_person_by_name_then_age;
  }
  qsort(arr, length, sizeof(struct aged_person*), cmp);
  // If you want, you could also use a ternary:
  // qsort(arr, length, sizeof(struct aged_person*), by_name_first ?
  //     compare_aged_person_by_name_then_age : compare_aged_person_by_age_then_name);
}
